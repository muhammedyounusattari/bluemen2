...
export class GradeStandingGroupListMoveBoxComponent implements OnInit {
  ...
  getDeletedItemById(id: any) {
    ...
    this._gradingGroupStandingService.getDeletedGradingGroupItemById(id).subscribe(result => {
      if (result && result != null) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          ...
          if (result1 == true) {
            this._gradingGroupStandingService.recoverGradingGroupList(currentData).subscribe(result2 => {
              if (result2) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          if (result1 == true) {
            this._gradingGroupStandingService.updateGradingGroupId(verifyData).subscribe(result3 => {
              if (result3) {
                this.dialogRef.close(true);
              }
            });
