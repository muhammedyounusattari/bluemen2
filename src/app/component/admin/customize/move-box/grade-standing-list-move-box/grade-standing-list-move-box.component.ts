    ...
    this._gradingGroupStandingService.getDeletedGradingStandingById(id).subscribe(result => {
      if (result && result != null) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          ...
          if (result1 == true) {
            this._gradingGroupStandingService.recoverGradingStandingList(currentData).subscribe(result2 => {
              if (result2) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          if (result1 == true) {
            this._gradingGroupStandingService.updateGradingStandingId(verifyData).subscribe(result3 => {
              if (result3) {
                this.dialogRef.close(true);
              }
            });
