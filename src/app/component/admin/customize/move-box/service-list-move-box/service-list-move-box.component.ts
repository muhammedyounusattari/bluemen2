...
export class ServiceListMoveBoxComponent implements OnInit {
  ...
  getDeletedItemById(id: any) {
    ...
    this._activityGroupServicesService.getDeletedActivityItemById(id).subscribe(result => {
      if (result && result != null) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          ...
          if (result1 == true) {
            this._activityGroupServicesService.recoverActivityList(currentData).subscribe(result2 => {
              if (result2) {
        ...
        confirmDialog.afterClosed().subscribe(result1 => {
          if (result1 == true) {
            this._activityGroupServicesService.updateActivityId(verifyData).subscribe(result3 => {
              if (result3) {
                this.dialogRef.close(true);
              }
            });
