...
import { DispTeacherContactsService } from 'src/app/services/teacher/disp-teacher-contacts.service';
...
@Component({
  selector: 'display-teacher-contacts',
  templateUrl: './display-teacher-contacts.component.html',
  styleUrls: ['./display-teacher-contacts.component.css'],
})
export class DisplayTeacherContactsComponent implements OnInit {
  ...
  @ViewChild('teacherStudentEditPopup')
  teacherStudentEditPopupRef: TemplateRef<any>;
  ...
  public togglePerOne(selected: string) {
    this.allSelected =
      this.teacherContactsModalForm.controls.siteLocation.value.length ==
  ...
  public openEditModal() {
    ...
    this.editModalRef = this.modalService.show(
      this.teacherStudentEditPopupRef,
      this.modalConfigSM
    );
  }
  ...
  public openActivityServiceModal(selector: string) {
    ...
    if (this.isActivityEdit) {
      ...
      this.activityServiceForm.patchValue({
        activity:
          this.teacherContactsEditModalForm.controls['activityService'].value,
        time: 0,
      });
      this.activityServiceModalRef = this.modalService.show(
        this.activityServicePopupRef,
   ...
   */
  public getDispTeacherContacts() {
    this.spinner = true;
    this.teacherContactsService
      .getDispTeacherContacts()
   ...
   */
  public addDispTeacherContacts() {
    this.teacherContactsService
      .addDispTeacherContacts(this.requestPayload())
      .subscribe((result: any) => {
        if (result) {
          this.getDispTeacherContacts();
        }
   ...
   */
  public editDispTeacherContacts(): any {
    ...
    this.teacherContactsService
      .editDispTeacherContacts(request)
      .subscribe((result: any) => {
        if (result) {
          this.getDispTeacherContacts();
        }
   ...
   */
  public deleteDispTeacherContacts() {
    ...
    confirmDialog.afterClosed().subscribe((result: any) => {
      if (result === true) {
        this.teacherContactsService
          .deleteDispTeacherContacts({ ssno: this.selectedRowData.ssno })
          .subscribe((result: any) => {
            if (result) {
              this.getDispTeacherContacts();
            }
  ...
  public handleMethodToCall() {
    switch (this.selectedOption) {
      case 'New':
        this.addDispTeacherContacts();
        break;
      case 'Edit':
        this.editDispTeacherContacts();
        break;
      default:
        break;
    }
  ...
  public addServiceActivity() {
    ...
    this.teacherContactsEditModalForm.controls['activityService'].setValue(
      null
    );
    let totalTime = 0;
    if (this.activityServiceData.length > 0) {
  ...
  public removeServiceActivity() {
    if (this.activityServiceData.length > 0) {
      ...
      this.teacherContactsEditModalForm.controls['totalTime'].setValue(
        totalTime
      );
    }
    this.serviceDataSource = new MatTableDataSource(this.activityServiceData);
  ...
  public requestPayload() {
    ...
    return {
      ...
      staffTotalTime:
        this.teacherContactsEditModalForm.controls.totalTime.value.toString(),
