...
import { DispCounselorContactsService } from 'src/app/services/counselor/disp-counselor-contacts.service';
...
@Component({
  selector: 'display-counselor-contacts',
  templateUrl: './display-counselor-contacts.component.html',
  styleUrls: ['./display-counselor-contacts.component.css'],
})
export class DisplayCounselorContactsComponent implements OnInit {
  public columnsToDisplay: string[] = [
    'firstName',
    'lastName',
    'phoneNumber',
    'staffContactDate',
    'staffContactCounselor',
    'staffTotalTime',
    'staffRecontactDate',
  ];
  ...
  @ViewChild('counselorStudentPopup')
  counselorStudentPopupRef: TemplateRef<any>;
  public modalRef: BsModalRef;
  @ViewChild('counselorStudentEditPopup')
  counselorStudentEditPopupRef: TemplateRef<any>;
  ...
  @ViewChild('activityTablePage', { read: MatPaginator, static: true })
  activityPaginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  @ViewChild('editPopup', { read: MatSort, static: true })
  editPopupSort: MatSort;
  @ViewChild('activityTable', { read: MatSort, static: true })
  activitySort: MatSort;
  ...
  public togglePerOne(selected: string) {
    this.allSelected =
      this.counselorContactsForm.controls.siteLocation.value.length ==
  ...
  public openEditModal() {
    ...
    this.editModalRef = this.modalService.show(
      this.counselorStudentEditPopupRef,
      this.modalConfigSM
    );
  }
  ...
  public openActivityServiceModal(selector: string) {
    ...
    if (this.isActivityEdit) {
      if (this.activityServiceData.length > 0) {
        ...
        this.activityServiceModalRef = this.modalService.show(
          this.activityServicePopupRef,
          this.modalConfigSM
        );
      }
    } else {
      this.activityServiceForm.patchValue({
        activity:
          this.counselorContactsEditModalForm.controls['activityService'].value,
        time: 0,
      });
      this.activityServiceModalRef = this.modalService.show(
        this.activityServicePopupRef,
   ...
   */
  public getDispCounselorContacts() {
    this.spinner = true;
    this.counselorContactsService
      .getDispCounselorContacts()
   ...
   */
  public addDispCounselorContacts() {
    this.counselorContactsService
      .addDispCounselorContacts(this.requestPayload())
      .subscribe((result: any) => {
        if (result) {
          this.getDispCounselorContacts();
        }
   ...
   */
  public deleteDispCounselorContacts() {
    ...
    confirmDialog.afterClosed().subscribe((result: any) => {
      if (result === true) {
        this.counselorContactsService
          .deleteDispCounselorContacts({ id: this.selectedRowData.id })
          .subscribe((result: any) => {
            if (result) {
              this.getDispCounselorContacts();
            }
  ...
  public handleMethodToCall() {
    switch (this.selectedOption) {
      case 'New':
        this.addDispCounselorContacts();
        break;
      case 'Edit':
        this.editDispCounselorContacts();
        break;
      default:
        break;
    }
  ...
  public addServiceActivity() {
    if (this.isActivityEdit) {
      this.activityServiceData[this.selectedEditModalRow] = {
        activity: this.activityServiceForm.value.activity,
        totalTime: this.activityServiceForm.value.time,
      };
    } else {
      this.activityServiceData.push({
        activity: this.activityServiceForm.value.activity,
        totalTime: this.activityServiceForm.value.time,
      });
    }
    this.counselorContactsEditModalForm.controls['activityService'].setValue(
      null
    );
    let totalTime = 0;
    if (this.activityServiceData.length > 0) {
    ...
    this.counselorContactsEditModalForm.controls['totalTime'].setValue(
      totalTime
    );
  ...
  public patchValuesToForm() {
    this.counselorContactsEditModalForm.patchValue({
      ...
      recontactDate: Utility.formatDate(
        this.selectedRowData?.staffRecontactDate
      ),
    ...
    this.activityServiceData = this.selectedRowData.activityRenderedList
      ? this.selectedRowData.activityRenderedList
      : [];
    this.getSelectedEditModalRow(this.activityServiceData[0], 0);
    this.serviceDataSource = new MatTableDataSource(this.activityServiceData);
  }
  ...
  public requestPayload() {
    ...
    return {
      ...
      staffTotalTime:
        this.counselorContactsEditModalForm.controls.totalTime.value.toString(),
