...
export class StaffContactsComponent {
    ...
    ['staffFirstName', 'staffLastName', 'staffPhoneNumber', 'staffContactDate', 'staffActive', 'staffContactTime', 'staffRecontactDate'];
    ...
    public staffContactsForm: FormGroup;
   ...
   */
   public initialiseForm() {
    ...
    this.staffContactsForm = this.fb.group({
      active: [''],
      contactTime: [''],
      firstName: [''],
      lastName: [''],
    ...
    public getStaffContacts() {
      ...
      this.staffContactsService.getStaffContacts().subscribe((result: any) => {
        if (result && result.length > 0) {
          ...
          // this.staffContactsList = result.map((x: any) => x = {...x, ...this.mockAPIResponse()});
          this.spinner = false;
    ...
    public getTStudentsList() {
      this.spinner = true;
      this.staffContactsService.getTStudentsList().subscribe((result: any) => {
        if (result) {
          this.spinner = false;
